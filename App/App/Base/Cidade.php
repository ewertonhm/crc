...
use App\App\Bairro as ChildBairro;
use App\App\BairroQuery as ChildBairroQuery;
use App\App\Cidade as ChildCidade;
use App\App\CidadeQuery as ChildCidadeQuery;
use App\App\Estado as ChildEstado;
use App\App\EstadoQuery as ChildEstadoQuery;
use App\App\Map\AtendimentoTableMap;
use App\App\Map\BairroTableMap;
...
abstract class Cidade implements ActiveRecordInterface
{
     ...
     * TableMap class name
     */
    const TABLE_MAP = '\\App\\App\\Map\\CidadeTableMap';
    ...
    /**
     * @var        ObjectCollection|ChildBairro[] Collection to store aggregation of ChildBairro objects.
     */
    protected $collBairros;
    protected $collBairrosPartial;
    ...
    /**
     * An array of objects scheduled for deletion.
     * @var ObjectCollection|ChildBairro[]
     */
    protected $bairrosScheduledForDeletion = null;

    /**
     * Initializes internal state of App\App\Base\Cidade object.
     */
     ...
     * @return $this|\App\App\Cidade The current object (for fluent API support)
     */
    ...
    public function setEstadoId($v)
    {
        ...
        if ($this->estado_id !== $v) {
            $this->estado_id = $v;
            $this->modifiedColumns[CidadeTableMap::COL_ESTADO_ID] = true;
        }
    ...
    public function hydrate($row, $startcol = 0, $rehydrate = false, $indexType = TableMap::TYPE_NUM)
    {
        try {
            ...
            $col = $row[TableMap::TYPE_NUM == $indexType ? 2 + $startcol : CidadeTableMap::translateFieldName('EstadoId', TableMap::TYPE_PHPNAME, $indexType)];
            ...
            throw new PropelException(sprintf('Error populating %s object', '\\App\\App\\Cidade'), 0, $e);
        }
    }
    ...
    protected function doSave(ConnectionInterface $con)
    {
        ...
        if (!$this->alreadyInSave) {
            ...
            if ($this->bairrosScheduledForDeletion !== null) {
                if (!$this->bairrosScheduledForDeletion->isEmpty()) {
                    foreach ($this->bairrosScheduledForDeletion as $bairro) {
                        // need to save related object because we set the relation to null
                        $bairro->save($con);
                    }
                    $this->bairrosScheduledForDeletion = null;
                }
            }

            if ($this->collBairros !== null) {
                foreach ($this->collBairros as $referrerFK) {
                    if (!$referrerFK->isDeleted() && ($referrerFK->isNew() || $referrerFK->isModified())) {
                        $affectedRows += $referrerFK->save($con);
                    }
                }
    ...
    protected function doInsert(ConnectionInterface $con)
    {
        ...
        if (null === $this->id) {
            try {
                $dataFetcher = $con->query("SELECT nextval('cidade_id_seq')");
        ...
        if ($this->isColumnModified(CidadeTableMap::COL_ESTADO_ID)) {
    ...
    public function toArray($keyType = TableMap::TYPE_PHPNAME, $includeLazyLoadColumns = true, $alreadyDumpedObjects = array(), $includeForeignObjects = false)
    {
        ...
        if ($includeForeignObjects) {
            ...
            if (null !== $this->collBairros) {
                ...
                $result[$key] = $this->collBairros->toArray(null, false, $keyType, $includeLazyLoadColumns, $alreadyDumpedObjects);
            }
        }
     ...
     * @param string $keyType The type of keys the array uses.
     *
     * @return $this|\App\App\Cidade The current object, for fluid interface
     */
    ...
    public function buildCriteria()
    {
        ...
        if ($this->isColumnModified(CidadeTableMap::COL_ESTADO_ID)) {
            $criteria->add(CidadeTableMap::COL_ESTADO_ID, $this->estado_id);
        }
     ...
     * objects.
     *
     * @param      object $copyObj An object of \App\App\Cidade (or compatible) type.
    ...
    public function copyInto($copyObj, $deepCopy = false, $makeNew = true)
    {
        ...
        if ($deepCopy) {
            ...
            foreach ($this->getBairros() as $relObj) {
                if ($relObj !== $this) {  // ensure that we don't try to copy a reference to ourselves
                    $copyObj->addBairro($relObj->copy($deepCopy));
                }
            }
     ...
     *
     * @param  boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
     * @return \App\App\Cidade Clone of current object.
     * @throws PropelException
     */
    ...
    public function setAtendimentos(Collection $atendimentos, ConnectionInterface $con = null)
    {
        ...
        foreach ($atendimentosToDelete as $atendimentoRemoved) {
            $atendimentoRemoved->setCidade(null);
        }
    ...
    /**
     * Clears out the collBairros collection
     *
     * This does not modify the database; however, it will remove any associated objects, causing
     * them to be refetched by subsequent calls to accessor method.
     *
     ...
     * @see        addBairros()
     */
    public function clearBairros()
    {
        $this->collBairros = null; // important to set this to NULL since that means it is uninitialized
    }

    /**
     * Reset is the collBairros collection loaded partially.
     */
    public function resetPartialBairros($v = true)
    {
        $this->collBairrosPartial = $v;
    }

    /**
     * Initializes the collBairros collection.
     *
     * By default this just sets the collBairros collection to an empty array (like clearcollBairros());
     * however, you may wish to override this method in your stub class to provide setting appropriate
     * to your application -- for example, setting the initial array to the values stored in database.
     *
     * @param      boolean $overrideExisting If set to true, the method call initializes
     ...
     * @return void
     */
    public function initBairros($overrideExisting = true)
    {
        if (null !== $this->collBairros && !$overrideExisting) {
            return;
        }

        $collectionClassName = BairroTableMap::getTableMap()->getCollectionClassName();

        $this->collBairros = new $collectionClassName;
        $this->collBairros->setModel('\App\App\Bairro');
    }

    /**
     * Gets an array of ChildBairro objects which contain a foreign key that references this object.
     *
     * If the $criteria is not null, it is used to always fetch the results from the database.
     * Otherwise the results are fetched from the database the first time, then cached.
     * Next time the same method is called without $criteria, the cached collection is returned.
     ...
     * @param      Criteria $criteria optional Criteria object to narrow the query
     * @param      ConnectionInterface $con optional connection object
     * @return ObjectCollection|ChildBairro[] List of ChildBairro objects
     * @throws PropelException
     */
    public function getBairros(Criteria $criteria = null, ConnectionInterface $con = null)
    {
        $partial = $this->collBairrosPartial && !$this->isNew();
        if (null === $this->collBairros || null !== $criteria  || $partial) {
            if ($this->isNew() && null === $this->collBairros) {
                // return empty collection
                $this->initBairros();
            } else {
                $collBairros = ChildBairroQuery::create(null, $criteria)
                    ->filterByCidade($this)
                    ->find($con);

                if (null !== $criteria) {
                    if (false !== $this->collBairrosPartial && count($collBairros)) {
                        $this->initBairros(false);

                        foreach ($collBairros as $obj) {
                            if (false == $this->collBairros->contains($obj)) {
                                $this->collBairros->append($obj);
                            }
                        }

                        $this->collBairrosPartial = true;
                    }

                    return $collBairros;
                }

                if ($partial && $this->collBairros) {
                    foreach ($this->collBairros as $obj) {
                        if ($obj->isNew()) {
                            $collBairros[] = $obj;
                        }
                    }
                ...
                $this->collBairros = $collBairros;
                $this->collBairrosPartial = false;
            }
        }

        return $this->collBairros;
    }

    /**
     * Sets a collection of ChildBairro objects related by a one-to-many relationship
     * to the current object.
     * It will also schedule objects for deletion based on a diff between old objects (aka persisted)
     * and new objects from the given Propel collection.
     *
     * @param      Collection $bairros A Propel collection.
     * @param      ConnectionInterface $con Optional connection object
     * @return $this|ChildCidade The current object (for fluent API support)
     */
    public function setBairros(Collection $bairros, ConnectionInterface $con = null)
    {
        /** @var ChildBairro[] $bairrosToDelete */
        $bairrosToDelete = $this->getBairros(new Criteria(), $con)->diff($bairros);


        $this->bairrosScheduledForDeletion = $bairrosToDelete;

        foreach ($bairrosToDelete as $bairroRemoved) {
            $bairroRemoved->setCidade(null);
        }
    ...
    /**
     * Returns the number of related Bairro objects.
     *
     * @param      Criteria $criteria
     * @param      boolean $distinct
     * @param      ConnectionInterface $con
     * @return int             Count of related Bairro objects.
     * @throws PropelException
     */
    public function countBairros(Criteria $criteria = null, $distinct = false, ConnectionInterface $con = null)
    {
        $partial = $this->collBairrosPartial && !$this->isNew();
        if (null === $this->collBairros || null !== $criteria || $partial) {
            if ($this->isNew() && null === $this->collBairros) {
            ...
            if ($partial && !$criteria) {
                return count($this->getBairros());
            }

            $query = ChildBairroQuery::create(null, $criteria);
            if ($distinct) {
                $query->distinct();
            }
        ...
        return count($this->collBairros);
    }

    /**
     * Method called to associate a ChildBairro object to this object
     * through the ChildBairro foreign key attribute.
     *
     * @param  ChildBairro $l ChildBairro
     * @return $this|\App\App\Cidade The current object (for fluent API support)
     */
    public function addBairro(ChildBairro $l)
    {
        if ($this->collBairros === null) {
            $this->initBairros();
            $this->collBairrosPartial = true;
        ...
        if (!$this->collBairros->contains($l)) {
            $this->doAddBairro($l);

            if ($this->bairrosScheduledForDeletion and $this->bairrosScheduledForDeletion->contains($l)) {
                $this->bairrosScheduledForDeletion->remove($this->bairrosScheduledForDeletion->search($l));
            }
        }
    ...
    /**
     * @param ChildBairro $bairro The ChildBairro object to add.
     */
    protected function doAddBairro(ChildBairro $bairro)
    {
        $this->collBairros[]= $bairro;
        $bairro->setCidade($this);
    }

    /**
     * @param  ChildBairro $bairro The ChildBairro object to remove.
     * @return $this|ChildCidade The current object (for fluent API support)
     */
    public function removeBairro(ChildBairro $bairro)
    {
        if ($this->getBairros()->contains($bairro)) {
            $pos = $this->collBairros->search($bairro);
            $this->collBairros->remove($pos);
            if (null === $this->bairrosScheduledForDeletion) {
                $this->bairrosScheduledForDeletion = clone $this->collBairros;
                $this->bairrosScheduledForDeletion->clear();
            }
            $this->bairrosScheduledForDeletion[]= $bairro;
    ...
    {
        if (null !== $this->aEstado) {
            $this->aEstado->removeCidade($this);
        }
        $this->id = null;
        $this->nome = null;
        $this->estado_id = null;
