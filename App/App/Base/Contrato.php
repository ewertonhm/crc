...
use \Exception;
use \PDO;
use App\App\ContratoQuery as ChildContratoQuery;
use App\App\Map\ContratoTableMap;
...
abstract class Contrato implements ActiveRecordInterface
{
     ...
     * TableMap class name
     */
    const TABLE_MAP = '\\App\\App\\Map\\ContratoTableMap';
    ...
    /**
     * Initializes internal state of App\App\Base\Contrato object.
     */
     ...
     * @return $this|\App\App\Contrato The current object (for fluent API support)
     */
    public function setId($v)
    {
        ...
        if ($this->id !== $v) {
            $this->id = $v;
            $this->modifiedColumns[ContratoTableMap::COL_ID] = true;
        }
    ...
    public function setContrato($v)
    {
        ...
        if ($this->contrato !== $v) {
            $this->contrato = $v;
            $this->modifiedColumns[ContratoTableMap::COL_CONTRATO] = true;
        }
    ...
    public function hydrate($row, $startcol = 0, $rehydrate = false, $indexType = TableMap::TYPE_NUM)
    {
        try {
            ...
            $col = $row[TableMap::TYPE_NUM == $indexType ? 0 + $startcol : ContratoTableMap::translateFieldName('Id', TableMap::TYPE_PHPNAME, $indexType)];
            $this->id = (null !== $col) ? (int) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 1 + $startcol : ContratoTableMap::translateFieldName('Contrato', TableMap::TYPE_PHPNAME, $indexType)];
            $this->contrato = (null !== $col) ? (string) $col : null;
            $this->resetModified();
            ...
            return $startcol + 2; // 2 = ContratoTableMap::NUM_HYDRATE_COLUMNS.

        } catch (Exception $e) {
            throw new PropelException(sprintf('Error populating %s object', '\\App\\App\\Contrato'), 0, $e);
        }
    }
    ...
    protected function doInsert(ConnectionInterface $con)
    {
        ...
        $this->modifiedColumns[ContratoTableMap::COL_ID] = true;
        if (null !== $this->id) {
            throw new PropelException('Cannot insert a value for auto-increment primary key (' . ContratoTableMap::COL_ID . ')');
        }
        if (null === $this->id) {
            try {
                $dataFetcher = $con->query("SELECT nextval('contrato_id_seq')");
        ...
        if ($this->isColumnModified(ContratoTableMap::COL_ID)) {
            $modifiedColumns[':p' . $index++]  = 'id';
        }
        if ($this->isColumnModified(ContratoTableMap::COL_CONTRATO)) {
     ...
     * @param string $keyType The type of keys the array uses.
     *
     * @return $this|\App\App\Contrato The current object, for fluid interface
     */
    ...
    public function buildCriteria()
    {
        ...
        if ($this->isColumnModified(ContratoTableMap::COL_ID)) {
            $criteria->add(ContratoTableMap::COL_ID, $this->id);
        }
        if ($this->isColumnModified(ContratoTableMap::COL_CONTRATO)) {
            $criteria->add(ContratoTableMap::COL_CONTRATO, $this->contrato);
        }
     ...
     * objects.
     *
     * @param      object $copyObj An object of \App\App\Contrato (or compatible) type.
     ...
     *
     * @param  boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
     * @return \App\App\Contrato Clone of current object.
     * @throws PropelException
     */
