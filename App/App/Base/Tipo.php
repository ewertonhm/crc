...
use App\App\Tipo as ChildTipo;
use App\App\TipoQuery as ChildTipoQuery;
use App\App\Map\AtendimentoTableMap;
use App\App\Map\TipoTableMap;
...
abstract class Tipo implements ActiveRecordInterface
{
     ...
     * TableMap class name
     */
    const TABLE_MAP = '\\App\\App\\Map\\TipoTableMap';
    ...
    /**
     * Initializes internal state of App\App\Base\Tipo object.
     */
     ...
     * @return $this|\App\App\Tipo The current object (for fluent API support)
     */
    public function setId($v)
    {
        ...
        if ($this->id !== $v) {
            $this->id = $v;
            $this->modifiedColumns[TipoTableMap::COL_ID] = true;
        }
    ...
    public function setTipo($v)
    {
        ...
        if ($this->tipo !== $v) {
            $this->tipo = $v;
            $this->modifiedColumns[TipoTableMap::COL_TIPO] = true;
        }
    ...
    public function hydrate($row, $startcol = 0, $rehydrate = false, $indexType = TableMap::TYPE_NUM)
    {
        try {
            ...
            $col = $row[TableMap::TYPE_NUM == $indexType ? 0 + $startcol : TipoTableMap::translateFieldName('Id', TableMap::TYPE_PHPNAME, $indexType)];
            ...
            throw new PropelException(sprintf('Error populating %s object', '\\App\\App\\Tipo'), 0, $e);
        }
    }
    ...
    protected function doInsert(ConnectionInterface $con)
    {
        ...
        $this->modifiedColumns[TipoTableMap::COL_ID] = true;
        if (null !== $this->id) {
            throw new PropelException('Cannot insert a value for auto-increment primary key (' . TipoTableMap::COL_ID . ')');
        }
        if (null === $this->id) {
            try {
                $dataFetcher = $con->query("SELECT nextval('tipo_id_seq')");
        ...
        if ($this->isColumnModified(TipoTableMap::COL_ID)) {
     ...
     * @param string $keyType The type of keys the array uses.
     *
     * @return $this|\App\App\Tipo The current object, for fluid interface
     */
    ...
    public function buildCriteria()
    {
        ...
        if ($this->isColumnModified(TipoTableMap::COL_ID)) {
            $criteria->add(TipoTableMap::COL_ID, $this->id);
        }
        if ($this->isColumnModified(TipoTableMap::COL_TIPO)) {
     ...
     * objects.
     *
     * @param      object $copyObj An object of \App\App\Tipo (or compatible) type.
     ...
     *
     * @param  boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
     * @return \App\App\Tipo Clone of current object.
     * @throws PropelException
     */
    ...
    public function setAtendimentos(Collection $atendimentos, ConnectionInterface $con = null)
    {
        ...
        foreach ($atendimentosToDelete as $atendimentoRemoved) {
            $atendimentoRemoved->setTipo(null);
        }
