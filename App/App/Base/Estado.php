...
use App\App\Map\EstadoTableMap;
...
abstract class Estado implements ActiveRecordInterface
{
     ...
     * TableMap class name
     */
    const TABLE_MAP = '\\App\\App\\Map\\EstadoTableMap';
    ...
    /**
     * @var        ObjectCollection|ChildCidade[] Collection to store aggregation of ChildCidade objects.
     */
    ...
    /**
     * An array of objects scheduled for deletion.
     * @var ObjectCollection|ChildCidade[]
     */
    protected $cidadesScheduledForDeletion = null;

    /**
     * Initializes internal state of App\App\Base\Estado object.
     */
     ...
     * @return $this|\App\App\Estado The current object (for fluent API support)
     */
    public function setId($v)
    {
        ...
        if ($this->id !== $v) {
            $this->id = $v;
            $this->modifiedColumns[EstadoTableMap::COL_ID] = true;
        }
    ...
    public function setNome($v)
    {
        ...
        if ($this->nome !== $v) {
            $this->nome = $v;
            $this->modifiedColumns[EstadoTableMap::COL_NOME] = true;
        }
    ...
    public function setUf($v)
    {
        ...
        if ($this->uf !== $v) {
            $this->uf = $v;
            $this->modifiedColumns[EstadoTableMap::COL_UF] = true;
        }
    ...
    public function hydrate($row, $startcol = 0, $rehydrate = false, $indexType = TableMap::TYPE_NUM)
    {
        try {
            ...
            $col = $row[TableMap::TYPE_NUM == $indexType ? 0 + $startcol : EstadoTableMap::translateFieldName('Id', TableMap::TYPE_PHPNAME, $indexType)];
            $this->id = (null !== $col) ? (int) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 1 + $startcol : EstadoTableMap::translateFieldName('Nome', TableMap::TYPE_PHPNAME, $indexType)];
            ...
            throw new PropelException(sprintf('Error populating %s object', '\\App\\App\\Estado'), 0, $e);
        }
    }
        ...
        if (!$this->alreadyInSave) {
            ...
            if ($this->cidadesScheduledForDeletion !== null) {
                if (!$this->cidadesScheduledForDeletion->isEmpty()) {
                    foreach ($this->cidadesScheduledForDeletion as $cidade) {
                        // need to save related object because we set the relation to null
                        $cidade->save($con);
                    }
    ...
    protected function doInsert(ConnectionInterface $con)
    {
        ...
        $this->modifiedColumns[EstadoTableMap::COL_ID] = true;
        if (null !== $this->id) {
            throw new PropelException('Cannot insert a value for auto-increment primary key (' . EstadoTableMap::COL_ID . ')');
        }
        if (null === $this->id) {
            try {
                $dataFetcher = $con->query("SELECT nextval('estado_id_seq')");
        ...
        if ($this->isColumnModified(EstadoTableMap::COL_ID)) {
            $modifiedColumns[':p' . $index++]  = 'id';
        }
        if ($this->isColumnModified(EstadoTableMap::COL_NOME)) {
    ...
    public function toArray($keyType = TableMap::TYPE_PHPNAME, $includeLazyLoadColumns = true, $alreadyDumpedObjects = array(), $includeForeignObjects = false)
    {
        ...
        if ($includeForeignObjects) {
            ...
            if (null !== $this->collCidades) {
                ...
                $result[$key] = $this->collCidades->toArray(null, false, $keyType, $includeLazyLoadColumns, $alreadyDumpedObjects);
            }
        }
     ...
     * @param string $keyType The type of keys the array uses.
     *
     * @return $this|\App\App\Estado The current object, for fluid interface
     */
    ...
    public function buildCriteria()
    {
        ...
        if ($this->isColumnModified(EstadoTableMap::COL_ID)) {
            $criteria->add(EstadoTableMap::COL_ID, $this->id);
        }
        if ($this->isColumnModified(EstadoTableMap::COL_NOME)) {
            $criteria->add(EstadoTableMap::COL_NOME, $this->nome);
        }
        if ($this->isColumnModified(EstadoTableMap::COL_UF)) {
     ...
     * objects.
     *
     * @param      object $copyObj An object of \App\App\Estado (or compatible) type.
     ...
     *
     * @param  boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
     * @return \App\App\Estado Clone of current object.
     * @throws PropelException
     */
    ...
    public function setAtendimentos(Collection $atendimentos, ConnectionInterface $con = null)
    {
        ...
        foreach ($atendimentosToDelete as $atendimentoRemoved) {
            $atendimentoRemoved->setEstado(null);
        }
    ...
    public function initCidades($overrideExisting = true)
    {
        ...
        $collectionClassName = CidadeTableMap::getTableMap()->getCollectionClassName();

        $this->collCidades = new $collectionClassName;
        $this->collCidades->setModel('\App\App\Cidade');
    }

    /**
     * Gets an array of ChildCidade objects which contain a foreign key that references this object.
     *
     * If the $criteria is not null, it is used to always fetch the results from the database.
     * Otherwise the results are fetched from the database the first time, then cached.
     * Next time the same method is called without $criteria, the cached collection is returned.
     ...
     * @param      Criteria $criteria optional Criteria object to narrow the query
     * @param      ConnectionInterface $con optional connection object
     * @return ObjectCollection|ChildCidade[] List of ChildCidade objects
     * @throws PropelException
     */
    public function getCidades(Criteria $criteria = null, ConnectionInterface $con = null)
    {
        ...
        if (null === $this->collCidades || null !== $criteria  || $partial) {
            if ($this->isNew() && null === $this->collCidades) {
                ...
                $collCidades = ChildCidadeQuery::create(null, $criteria)
                    ->filterByEstado($this)
                    ->find($con);
    ...
    /**
     * Sets a collection of ChildCidade objects related by a one-to-many relationship
     * to the current object.
     * It will also schedule objects for deletion based on a diff between old objects (aka persisted)
     * and new objects from the given Propel collection.
     *
     * @param      Collection $cidades A Propel collection.
     * @param      ConnectionInterface $con Optional connection object
     * @return $this|ChildEstado The current object (for fluent API support)
     */
    public function setCidades(Collection $cidades, ConnectionInterface $con = null)
    {
        /** @var ChildCidade[] $cidadesToDelete */
     ...
     * @throws PropelException
     */
    public function countCidades(Criteria $criteria = null, $distinct = false, ConnectionInterface $con = null)
    {
        $partial = $this->collCidadesPartial && !$this->isNew();
        if (null === $this->collCidades || null !== $criteria || $partial) {
            if ($this->isNew() && null === $this->collCidades) {
            ...
            $query = ChildCidadeQuery::create(null, $criteria);
            if ($distinct) {
                $query->distinct();
            }
    ...
    /**
     * Method called to associate a ChildCidade object to this object
     * through the ChildCidade foreign key attribute.
     *
     * @param  ChildCidade $l ChildCidade
     * @return $this|\App\App\Estado The current object (for fluent API support)
     */
    public function addCidade(ChildCidade $l)
    {
        if ($this->collCidades === null) {
            $this->initCidades();
            $this->collCidadesPartial = true;
        ...
        if (!$this->collCidades->contains($l)) {
            $this->doAddCidade($l);
    ...
    /**
     * @param ChildCidade $cidade The ChildCidade object to add.
     */
    protected function doAddCidade(ChildCidade $cidade)
    {
    ...
    /**
     * @param  ChildCidade $cidade The ChildCidade object to remove.
     * @return $this|ChildEstado The current object (for fluent API support)
     */
    public function removeCidade(ChildCidade $cidade)
    {
        if ($this->getCidades()->contains($cidade)) {
            $pos = $this->collCidades->search($cidade);
            $this->collCidades->remove($pos);
            ...
            $this->cidadesScheduledForDeletion[]= $cidade;
